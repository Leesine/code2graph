...
keywords = frozenset({
    ...
    'void', 'volatile', 'wchar_t', 'while', 'xor', 'xo r_eq', 'NULL'
})
...
def clean_gadget(gadget: List[str]):
    ...
    rx_fun = re.compile(r'\b([_A-Za-z]\w*)\b(?=\s*\()')  # 函数名正则表达式
    # regular expression to find variable name candidates
    # rx_var = re.compile(r'\b([_A-Za-z]\w*)\b(?!\s*\()')
    rx_var = re.compile(
        r'\b([_A-Za-z]\w*)\b(?:(?=\s*\w+\()|(?!\s*\w+))(?!\s*\()')  # 变量名正则表达式
    ...
    for line in gadget:
        ...
        # remove all string literals (keep the quotes)    去除所有字符串常量   ""里面字符的都被干掉了  literals: 字面值
        nostrlit_line = re.sub(r'".*?"', '""', line)
        # remove all character literals                         ''里面字符的都被干掉了， 去除所有字符常量
        nocharlit_line = re.sub(r"'.*?'", "''", nostrlit_line)
        # replace any non-ASCII characters with empty string
        ascii_line = re.sub(r'[^\x00-\x7f]', r'', nocharlit_line)  # 将所有非ascii码置空

        # return, in order, all regex matches at string list; preserves order for semantics
        user_fun = rx_fun.findall(ascii_line)  # 函数名
        user_var = rx_var.findall(ascii_line)  # 变量名
        ...
        for fun_name in user_fun:
            ...
            if len({fun_name}.difference(main_set)) != 0 and len({fun_name}.difference(keywords)) != 0:
                ...
                if fun_name not in fun_symbols.keys():  # fun_symbols: {为归一化前的函数名: FUN0, ...}
        ...
        for var_name in user_var:
            ...
            if len({var_name}.difference(keywords)) != 0 and len({var_name}.difference(main_args)) != 0:  # 不替换argc和argv
                # DEBUG
                ...
                if var_name not in var_symbols.keys():  # var_symbols: {为归一化的变量名: var1, ...}
...
def tokenize_code_line(line: str, subtoken: bool):
    ...
    while i < len(line):
        ...
        if line[i] == ' ':  # 遇到空格则将之前遍历到的字母组装成单词
            tmp.append(''.join(w).strip())
            tmp.append(line[i].strip())
            w = []
            i += 1
        # Check operators and append to final list
        elif line[i:i + 3] in operators3:  # 遇到>>=或者<<=则将之前遍历到的字母组装成单词
            tmp.append(''.join(w).strip())
            tmp.append(line[i:i + 3].strip())  # 将>>=或者<<=也加到打tmp
            w = []
            i += 3
