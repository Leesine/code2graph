...
def parse_source_code_file(input_dir, joern_parse_tool, joern_export_tool):
    ...
    for source_data_path in tqdm.tqdm(source_data_paths):                  # 为每一个源代码文件都生成对应的cpg.bin，以及对应的.dot文件
        cpg_bin_file = os.path.join(args.output_dir, os.path.join(os.path.dirname(source_data_path), "cpg.bin"))
        ...
        if not os.path.exists(cpg_bin_file):       # cpg.bin 没有生成
...
def parse_source_code_dir(input_dir, joern_parse_tool, joern_export_tool):
    ...
    for source_data_dir in tqdm.tqdm(source_data_dirs):                  # 为每一个源代码文件都生成对应的cpg.bin，以及对应的.dot文件
